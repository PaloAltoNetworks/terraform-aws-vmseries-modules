...
| <a name="input_managed_prefix_list"></a> [managed\_prefix\_list](#input\_managed\_prefix\_list) | Managed prefix list, which needs to be created and set as a destination in route | <pre>object({<br>    name        = string<br>    max_entries = number<br>    entries = map(object({<br>      cidr        = string<br>      description = string<br>    }))<br>  })</pre> | `null` | no |
