...
variable "type" {
  description = <<-EOF
  The type of the service. The type "Gateway" does not tolerate inputs `subnets`,  `security_group_ids`, and `private_dns_enabled`.
  ...
  If null, uses "Gateway".
  EOF
  type        = string
}

variable "create" {
  description = "If false, does not create a new AWS VPC Endpoint, but instead reads a pre-existing one. The inputs `name`, `tags`, `type` and `vpc_id` are used to match the pre-existing endpoint."
  default     = true
  type        = bool
}
