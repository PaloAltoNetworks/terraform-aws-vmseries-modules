...
resource "aws_network_interface" "app_nic" {
  ...
  security_groups = [module.app_vpc.security_group_ids["app_example"]]
  tags            = merge({ Name = "${var.prefix}nic-${each.key}" }, var.global_tags)
}
...
resource "aws_instance" "app_vm" {
  ...
  network_interface {
    device_index         = 0
    network_interface_id = aws_network_interface.app_nic[each.key].id
  }
}
