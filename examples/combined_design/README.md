...
| <a name="input_name_templates"></a> [name\_templates](#input\_name\_templates) | Templates for names defined by object with 2 attributes:<br>- name\_delimiter - it specifies the delimiter used between all components of the new name.<br>- name\_template - a list of maps, where keys are informational only. | <pre>object({<br>    name_delimiter  = string<br>    name_template   = map(list(map(string)))<br>    assign_template = map(string)<br>  })</pre> | n/a | yes |
