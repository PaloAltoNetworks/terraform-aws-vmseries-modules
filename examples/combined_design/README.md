...
| <a name="input_name_templates"></a> [name\_templates](#input\_name\_templates) | Templates for names defined by object with 2 attributes:<br>- name\_template - a list of maps, where keys are informational only.<br>- assigned\_template - map of templates assigned to each kind of resources | <pre>object({<br>    name_template     = map(list(map(string)))<br>    assigned_template = map(string)<br>  })</pre> | n/a | yes |
